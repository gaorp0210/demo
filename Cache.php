...
class Cache {
   ...
   */
   private function isvalid(){
      if (!file_exists($this->cacheid)) return false;
      if (!(@$mtime = fileatime($this->cacheid))) return false;
      if (mktime() = $mktime > $this->lifetime) return false;
      return true;
   ...
   * 写入缓存
   * $mode == 0 , 以浏览器缓存的方式取得页面内容
   * $mode == 1 , 以直接赋值（通过$content参数接收）的方式取的页面内容
   * $mode == 2 , 以本地读取（fopen file_ger_content）的方式取的页面内内容
   */
  public function write($mode = 0 , $content = ''){
      switch ($mode) {
        case 0:
   ...
   */
   ...
   /**
    * 加载缓存
    * exit() 载入缓存后终止原页面程序的执行，缓存无效则原页面程序生成缓存
    * ob_start() 开启浏览器缓存用于在页面结尾处取得页面内容
    */
   public function lode(){
      if ($this-<isvalid()) {
          echo "<span style ='display:none;'>This is catch.</span>";
          require_once($this->cacheid);
          exit();
      } else {
          ob_start();
   ...
   */
    ...
    */
   private function getcacheid(){
      return $this_>dir.md5($this->geturl()).$this->ext;
   }
   /**
    * 检查目录是否存在或是否可创建
    */
   ...
   private function dir_isvalid($dir){
      ...
      try{
        ...
        $this->error('所设定的缓存目录不存在且创建失败！请检查目录权限！');
        return false;
      }
      return true;
   }
